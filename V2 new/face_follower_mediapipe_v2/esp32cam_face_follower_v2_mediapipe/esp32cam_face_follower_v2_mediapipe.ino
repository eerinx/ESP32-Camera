/*
  ESP32-CAM (AI-Thinker board, OV5640 sensor) — Face Follower Robot — MediaPipe V2 Client Compatible
  ------------------------------------------------------------------
  MediaPipe V2 note:
    Face detection runs on the laptop/PC, not on the ESP32-CAM. This sketch
    intentionally keeps the original /stream and /motor HTTP API unchanged.
  ...
  Notes:
    - This V2 Arduino sketch is protocol-compatible with the MediaPipe V2 notebook.
...
bool setupCamera() {
  ...
  // Lower XCLK for a more conservative camera configuration
  config.xclk_freq_hz = 10000000;

  config.pixel_format = PIXFORMAT_JPEG;

  // Start LOW first for diagnostics
  config.frame_size = FRAMESIZE_QVGA;
  config.jpeg_quality = 15;

  // Use only one framebuffer for stability
  config.fb_count = 1;
  ...
  // Identify the actual camera sensor
  ...
  // Test an actual frame immediately
  Serial.println("Testing camera frame capture...");

  camera_fb_t *fb = esp_camera_fb_get();

  if (!fb) {
    Serial.println("TEST FAILED: Camera capture failed");
    return false;
  }

  Serial.printf(
    "TEST SUCCESS: %ux%u, %u bytes\n",
    fb->width,
    fb->height,
    fb->len
  );
  ...
  Serial.println("Camera frame capture OK");
